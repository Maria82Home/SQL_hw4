...
SELECT one.MARK, (SELECT COUNT(MARK) FROM auto two WHERE one.MARK <> two.MARK) as anothermark
FROM auto one
GROUP BY MARK;

CREATE TABLE test_a (id INT, data VARCHAR(50));
